...
%% Trial vs Trial Video-Feature Data
OM = [0.3 0.3 0.3];
RD = [0.01 0.87 0.02];
hm = [0.77 0.1 0.37];
lm = [0.2 0.28 0.45];
...
for v = 1:length(variables)
    ...
    lme = lme_models_seekvsabstain{v};

% --- CONTRASTS: TrialType effect within each group ---
% Adjust indices if coefficient order differs!

% EtOH/EtOH (reference group)
c1 = [0 1 0 0 0 0];
...
% Run tests
[p1,F1] = coefTest(lme, c1);
[p2,F2] = coefTest(lme, c2);
[p3,F3] = coefTest(lme, c3);

% Convert F to t (since 1 df contrast: t^2 = F)
t1 = sign(lme.Coefficients.Estimate(2)) * sqrt(F1);
t2 = sign(lme.Coefficients.Estimate(2) + lme.Coefficients.Estimate(5)) * sqrt(F2);
t3 = sign(lme.Coefficients.Estimate(2) + lme.Coefficients.Estimate(6)) * sqrt(F3);
...
% Display
fprintf('\n--- %s ---\n', variables{v});
fprintf('EtOH/EtOH: t(%d)=%.2f, p=%.3e (corr=%.3e)\n', df, t1, p1, pvals_corr(1));
fprintf('EtOH/Suc:  t(%d)=%.2f, p=%.3e (corr=%.3e)\n', df, t2, p2, pvals_corr(2));
fprintf('NoEtOH/Suc:t(%d)=%.2f, p=%.3e (corr=%.3e)\n', df, t3, p3, pvals_corr(3));
end
